  ...
  module Types
    module Canvas
      class CanvasLineItemInput < BaseInputObject
        ...
        argument :lti_submission_type, String, "The extension that defines the submission_type of the line_item. Only returns if set through the line_item create endpoint..Example: { 	'type':'external_tool', 	'external_tool_url':'https://my.launch.url', }", required: false
        argument :lti_launch_url, String, "The launch url of the Line Item. Only returned if `include=launch_url` query parameter is passed, and only for Show and List actions..Example: https://my.tool.url/launch", required: false
